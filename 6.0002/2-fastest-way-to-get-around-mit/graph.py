...
class WeightedEdge(Edge):
    ...
    def __str__(self):
        return '{0}->{1} ({2}, {3})'.format(self.get_source(), self.get_destination(), self.get_total_distance(), self.get_outdoor_distance())
...
class Digraph(object):
    ...
    def add_node(self, node):
        ...
        if self.has_node(node): raise ValueError
        self.nodes.add(node)
        self.edges[node] = []
    ...
    def add_edge(self, edge):
        ...
        if not self.has_node(destination) or not self.has_node(source): raise ValueError

        source_edges = self.get_edges_for_node(destination)
        destination_edges = self.get_edges_for_node(source)

        if edge not in source_edges: self.edges[source].append(edge)
        if edge not in destination_edges: self.edges[destination].append(edge)
